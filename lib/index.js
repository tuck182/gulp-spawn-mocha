...
module.exports = function (ops, coverage) {
  ...
  var stream = through(function (file) {
    ...
    if (istanbul) {
      ...
      bin = istanbul.bin || join(require.resolve('nyc'), '..', require('nyc/package.json').bin.nyc);
      // If the value for istanbul is literally true, just keep the arguments array. Otherwise, parse istanbul options.
      args = _.isObject(istanbul) ? parseArgs(_.omit(istanbul, ['bin'])).concat(args) : args;
    }
...
/**
 * Adds a given argument with name and value to argument array.
...
function addArg(args, name, val) {
  ...
  if (_.isString(val) || _.isNumber(val)) {
    ...
    args.push(_.map(val, (v, k) => `${k}=${v}`).join(','));
  }
}
